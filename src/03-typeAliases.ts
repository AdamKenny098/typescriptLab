...
// Use a BuddyList : Return a Friend Array
function getBuddyListFriends(list: BuddyList): Friend[] {
  return list.members.reduce<Friend[]>((friendArray, buddy) => { //reduce member of a BuddyList to a Friend[]
    if ("phone" in buddy) {
      friendArray.push(buddy as Friend); // add if has phone
    }
    return friendArray;
  }, []);
}
